...
const LogExportDialog = ({ sessions, onClose }) => {
  ...
  const handleExport = async () => {
    ...
    setExporting(true);
    try {
      const resp = await exportSessions(selectedIds, format);
      ...
      const filename = `mds_logs_export_${new Date().toISOString().slice(0, 19).replace(/:/g, '')}.${ext}`;
    ...
    <div className="log-export-overlay" onClick={onClose}>
      <div className="log-export-dialog" onClick={e => e.stopPropagation()}>
        ...
        <div className="export-actions">
          ...
          <button onClick={handleExport} disabled={exporting} className="active">
